      ...
      <div class="card">
        <div class="card-body p-5">
          ...
          <table class="table">
            <thead>
              <tr>
                <th>
                  <input
                    type="checkbox"
                    @click="checkAll ? uncheckAll() : toggleCheckAll()"
                  />
                </th>
            ...
            <tbody>
              <tr
                v-for="project in searchQuery ? searchedprojects : projects"
                :key="project.id"
                :class="{
                  'selected-task': selectedProjects.includes(project.id),
                }"
              >
                <td>
                  <input
...
const deleteTask = async (taskId) => {
  try {
    const response = await axios.delete(`project/${taskId}`);
    if (response.status === 200) {
